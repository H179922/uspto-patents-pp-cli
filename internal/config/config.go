...
type Config struct {
	...
	Path           string `toml:"-"`
	UsptoApiKey string `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("USPTO_PATENTS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "uspto-patents-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("USPTO_API_KEY"); v != "" {
		cfg.UsptoApiKey = v
		cfg.AuthSource = "env:USPTO_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.UsptoApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("USPTO_PATENTS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
