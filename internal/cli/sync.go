...
Exit codes & warnings:
  ...
  uspto-patents-pp-cli sync

  # Sync specific resources only
  uspto-patents-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  uspto-patents-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  uspto-patents-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  uspto-patents-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  uspto-patents-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "patent":
		return db.UpsertPatent(data)
	case "petition":
		return db.UpsertPetition(data)
	default:
...
func defaultSyncResources() []string {
	return []string{
		"datasets",
		"patent",
		"patent-appeals-decisions-search-download",
		"patent-applications-search-download",
		"patent-interferences-decisions-search-download",
		"patent-trials-decisions-search-download",
		"patent-trials-documents-search-download",
		"patent-trials-proceedings-search-download",
		"petition",
