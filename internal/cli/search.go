...
In local mode: searches locally synced data only.`,
  ...
  uspto-patents-pp-cli search "error timeout"

  # Force local search only
  uspto-patents-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  uspto-patents-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  uspto-patents-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/v1/datasets/products/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/api/v1/datasets/products/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
